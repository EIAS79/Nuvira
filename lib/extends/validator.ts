  ...
  private async validateSchema(
    ...
    const validateType = (expectedTypes: AllowedTypes[], value: any): boolean => {
      for (const type of expectedTypes) {
        ...
        if (type === 'StringArray' && Array.isArray(value) && value.every(v => typeof v === 'string')) return true;
        if (type === 'NumberArray' && Array.isArray(value) && value.every(v => typeof v === 'number')) return true;
        if (type === 'ObjectArray' && Array.isArray(value)) return true;
